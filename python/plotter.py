...
samples = {
    "W": ["QCD", "ST_tW_top", "ST_tW_antitop", "TTToSemiLeptonic", "TTToHadronic",  "ZJetsUnmatched", "ZJetsMatched",
          "WJetsUnmatched", "WJetsMatched"],
    "Zbb": ["QCD", "ST_tW_top", "ST_tW_antitop", "TTToSemiLeptonic", "TTToHadronic",  "ZJetsUnmatched", "ZJetsMatched",
            "WJetsUnmatched", "WJetsMatched"],
    "WfromTop": ["QCD",  "ST_sch",  "ST_tch", "ST_tWch",  "DYJets",  "WJets",  "TTbar_had", "TTbar_dilep",
                 "TTbar_semilep"],
    ...
    "top": ["QCD", "ST_s",  "ST_t", "ST_tW",  "DYJets",  "WJets",  "TTToHadronic", "TTTo2L2Nu",
            "TTToSemiLeptonic_mergedTop", "TTToSemiLeptonic_mergedW",  "TTToSemiLeptonic_mergedQB",
            "TTToSemiLeptonic_semiMergedTop",  "TTToSemiLeptonic_notMerged"]
}
...
mc_samples = {
    ...
    "top": ["other_ttbar",  "WJets",  "ST", "TTbarNotMerged",  "TTToSemiLeptonic_mergedQB", "TTToSemiLeptonic_mergedW",
            "TTToSemiLeptonic_mergedTop"],
}

merged_hists = {
    "TTbar": ["TTbar_had", "TTbar_dilep", "TTbar_semilep_mergedTop",  "TTbar_semilep_mergedW",
              "TTbar_semilep_mergedQB", "TTbar_semilep_semiMergedTop",  "TTbar_semilep_notMerged"],
...
pt_bins_dict = {
    ...
    "top": {
        "jms": ["inclusive", "200to300", "300to400", "400to500", "500to650", "650toInf"],
        "unfolding": ["inclusive", "200to300", "300to400", "400to500", "500to650", "650toInf"],
    },
    "W": {
        "jms": [
            "inclusive", "500to650", "650to800", "800to1200", "1200toInf",
        ],
        "unfolding": [
            "inclusive", "500to575", "575to650", "650to725", "725to800", "800to1000", "1000to1200", "1200toInf",
        ]
    },
    "Zbb": {
        "jms": [
...
for selection, workflows in pt_bins_dict.items():
    for workflow, bins in workflows.items():
        for pt_bin in bins:
            pt_bins_tex_dict[pt_bin] = "" if (pt_bin == "inclusive") \
    ...
    if (not pseudo_data):
        for data_name in ['Data', 'data_obs', 'data']:
            ...
            try:
                h_data.GetName()
                break
            except BaseException('tried getting data hist with', data_name,
                                 '(which failed miserably)') as e:
                logger.exception(e)
    ...
    for sample in samples:
        ...
        if (sample in merged_hists and include_merged_hists):
            for subsample in merged_hists[sample]:
                ...
                except BaseException("hist", str(hist_dir % subsample), 'not found. Taking empty hist instead.') as e:
                    logger.exception(e)
...
def plot_data_mc(h_data=None,
    ...
    cms_style.draw_lumi(plotpad,
                        year=year, lumi=lumis[year],
                        do_extra_text=draw_extra_text,
                        out_of_frame=True,
                        do_cms_text=draw_extra_text, private_work=private_work
                        )
    plotpad.cd()
    ...
    if (type(h_mc) is ROOT.THStack):
        ...
        raise NotImplementedError(
            "Type conversion for MC Hist(s) is not implementd (provided type: "+str(type(h_mc))+")"
        )
    ...
    for i in range(len(additional_text.split("\\"))):
        additional_latex.DrawLatex(
            ...
            cms_style.additional_text_ypos-1.1*i*cms_style.additional_text_size*additional_text_size_modifier,

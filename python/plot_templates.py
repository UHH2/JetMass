...
response_samples = {
    "W": ["ST_tW_top", "ST_tW_antitop", "TTToSemiLeptonic", "TTToHadronic",  "ZJetsUnmatched", "ZJetsMatched",
          "WJetsUnmatched", "WJetsMatched", "QCD"],
...
def plot_mass(selection: str,
    ...
    for ipt_bin in range(len(pt_bins)):
        for inbjet_bin in range(len(nbjet_bins)):
            ...
            regions = ['pass', 'passW', 'fail'] if selection.lower() == 'top' else ['pass', 'fail']

            for region in regions:
                legend_entries = []
                hist_dir = selection+'_%s__'+mass_name+'_'+pt_bin+("" if nbjet_bin == "" else "_"+nbjet_bin) + \
                    ('_'+region if len(region) > 0 else '')
                h_data, mc_hists = plotter.get_hists(f_hists, plotter.mc_samples[selection],
                                                     hist_dir, selection, scaleQCD=scaleQCD)
                h_data.GetXaxis().SetTitle("m_{SD}")
                bkg_stack = ROOT.THStack()
                for sample in plotter.mc_samples[selection]:
                    mc_hists[sample].SetFillColorAlpha(plotter.colors.get(sample), .8)
                    ...
                    legend_entries.append((mc_hists[sample], plotter.legend_names[sample], "f"))
                ...
                bininfo_tex = plotter.pt_bins_tex_dict[pt_bin] + \
                    ("" if nbjet_bin == "" else "\\"+plotter.nbjet_bins_tex_dict[nbjet_bin])
                additional_text = ' '+plotter.selection_tex[selection]+"\\" + \
                    bininfo_tex+"\\ %s #bf{%s}" % (plotter.region_tex[selection][region], 'prefit')
                ...
                plotter.plot_data_mc(h_data, bkg_stack, hist_dir.replace("_%s", ""), out_dir=output_dir,
                                     legend_entries=legend_entries, additional_text=additional_text)
...
if (__name__ == '__main__'):
    ...
    for selection in ['W', 'top']:
        ...
        plot_mass(selection, 'mjet', hist_file,
                  '../Plots/%s_coffea%s/' % (args.input.replace('.root', ''),
                                             '_originalQCDScale' if args.origQCDScale else ''),
                  binning="CMS", scaleQCD=not args.origQCDScale)
    # for selection in ['W']:

    ...
    for ipt_bin in range(len(pt_bins)):
        for inbjet_bin in range(len(nbjet_bins)):
            ...
            for region in regions:
                ...
                plotter.logY=logY
                bininfo_tex = plotter.pt_bins_tex_dict[pt_bin] + ("" if nbjet_bin == "" else "\\"+plotter.nbjet_bins_tex_dict[nbjet_bin])
                additional_text = ' '+plotter.selection_tex[selection]+"\\"+bininfo_tex+"\\ %s #bf{%s}"%(plotter.region_tex[selection][region],'prefit')
                ...
                plotter.plot_data_mc(h_data,bkg_stack,hist_dir.replace("_%s",""),out_dir=output_dir,legend_entries=legend_entries,additional_text=additional_text)

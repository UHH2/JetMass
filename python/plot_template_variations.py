...
def plot_template_variations(c_name,c_grid,file_name='../macros/Histograms.root',selection='top',output_dir='template_variations',variation=1):
                ...
                for j in range(len(c_grid['etabins'])-1):
                    ...
                    for pfflavour in c_grid['pfflavours']:
                        ...
                        for sample,hist in h_mc.items():
                            ...
                            add_text = plotter.selection_tex[selection]+'\\'+plotter.legend_names[sample]+'\\'+plotter.pt_bins_tex_dict[pt_bin]
                    ...
                    for sample,hists in all_mc.items():
                        ...
                        c,_,out_name = plotter.plot_data_mc(h_data=hists['nominal'], h_mc=None ,plot_title=selection+'_'+sample + ' '+pt_bin+' '+region[1:]+' all',out_dir=region_out_dir,additional_hists=all_variations,legend_entries=legend_entries,additional_text=add_text)
                        legend.Draw('SAME')
...
def plot_variation_envelope(c_name,c_grid,file_name='../macros/Histograms.root',selection='top',output_dir='template_variations_envelope'):
    f_hists = ROOT.TFile(file_name,'READ')
    if(not os.path.exists(output_dir)):
        os.makedirs(output_dir)
            ...
            for i in range(len(c_grid['ptbins'])-1):
                for j in range(len(c_grid['etabins'])-1):
                    ...
                    for sample, hist in h_mc.items():
                        ...
                        c = plotter.plot_data_mc(h_data=hist, h_mc=None ,plot_title=selection+' '+sample + ' '+pt_bin+' '+region,out_dir=region_out_dir,additional_hists=variation_bands[sample].values(),legend_entries=list(reversed(legend_entries)),additional_text=plotter.selection_tex[selection]+'\\'+plotter.legend_names[sample]+'\\'+plotter.pt_bins_tex_dict[pt_bin])

                    legend_entries=[(h_pseudo,'nominal (PseudoData)','l')]
                    for flavour in c_grid['pfflavours']:
                        h = variation_bands['pseudo'][flavour]
                        legend_entries.append((h,'#pm 1%% variation %s'%flavour,'f'))
                    plotter.plot_data_mc(h_data=h_pseudo, h_mc=None ,plot_title=selection+' '+'PseudoData' + ' '+pt_bin.replace('to','To')+' '+region,out_dir=region_out_dir,additional_hists=variation_bands['pseudo'].values(),legend_entries=list(reversed(legend_entries)),additional_text=plotter.selection_tex[selection]+'\\'+'PseudoData\\'+plotter.pt_bins_tex_dict[pt_bin])

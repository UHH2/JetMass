...
def plot_template_variations(
    ...
    colors = {
        "chargedH": ROOT.kRed - 4,
        "neutralH": ROOT.kAzure + 7,
        "gamma": 798,
        "other": 13,
        "all": ROOT.kRed - 4,
    }
            ...
            for i in range(len(c_grid["ptbins"]) - 1):
                for j in range(len(c_grid["etabins"]) - 1):
                    ...
                    for pfflavour in c_grid["pfflavours"]:
                        ...
                        h_pseudo, h_mc = plotter.get_hists(
                            f_hists,
                        ...
                        for direction in ["up", "down"]:
                            ...
                            varied_hists[direction] = plotter.get_hists(
                                f_hists,
                                plotter.mc_samples[selection],
                                hist_dir_varied,
                                pseudo_data=True,
                        ...
                        for sample, hist in h_mc.items():
                            if sample not in all_mc.keys():
                                all_mc[sample] = {}
                            variations = [
                                varied_hists[direction][1][sample]
                                for direction in ["up", "down"]
                            ]
                            ...
                            legend_entries.append(
                                (variations[0], "+%.1f%% variation " % variation, "l")
                            )
                            legend_entries.append(
                                (variations[1], "-%.1f%% variation " % variation, "l")
                            )
                        ...
                        variations = [
                            varied_hists[direction][0] for direction in ["up", "down"]
                        ]
                        variations[0].SetLineColor(ROOT.kBlue + 2)
                        variations[1].SetLineColor(ROOT.kRed + 2)
                    ...
                    for sample, hists in all_mc.items():
                        ...
                        for pfflavour in c_grid["pfflavours"]:
                            ...
                            up.SetLineColorAlpha(colors[pfflavour], 0.99)
                            up.SetLineStyle(1)
                            up_var_pf = up.Clone()
                            ...
                            max_y = max(
                                max_y,
                                max(
                                    abs(up_var_pf.GetMaximum()),
                                    abs(up_var_pf.GetMinimum()),
                                ),
                            )
                            up_var.Add(up_var_pf, "Hist")
                            down = hists[pfflavour][1]
                            down.SetLineColorAlpha(colors[pfflavour], 0.99)
                            down.SetLineStyle(2)
                            ...
                            max_y = max(
                                max_y,
                                max(
                                    abs(down_var_pf.GetMaximum()),
                                    abs(down_var_pf.GetMinimum()),
                                ),
                            )
                            ...
                            down.SetLineWidth(2)
                            legend_entries.append(
                                (up, "+%.1f%% variation " % variation + pfflavour, "l")
                            )
                            legend_entries.append(
                                (
                                    down,
                                    "-%.1f%% variation " % variation + pfflavour,
                                    "l",
                                )
                            )
                            # up.SetLineWidth(1)
                        ...
                        c, _, out_name = plotter.plot_data_mc(
                            h_data=hists["nominal"],
                            h_mc=None,
                            plot_title=selection
                            + "_"
                            + sample
                            + " "
                            + pt_bin
...
def build_asymmerrors(hists):
    ...
    for i in range(0, nbins):
        ...
        graph.SetPoint(
            nbins + i,
            nominal.GetBinCenter(nbins - ibin + 1),
            low.GetBinContent(nbins - ibin + 1),
        )
    return graph


def plot_variation_envelope(
    c_name,
    c_grid,
    file_name="../macros/Histograms.root",
    selection="top",
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    colors = {
        "chargedH": ROOT.kGreen + 2,
        "neutralH": ROOT.kRed + 2,
        "gamma": ROOT.kOrange,
        "other": ROOT.kBlue + 2,
    }
    ...
    for ipt_bin in range(len(plotter.pt_bins_dict[selection])):
        ...
        for region in (
            ["_pass", "_passW", "_fail"] if selection == "top" else ["_pass", "_fail"]
        ):
            ...
            for i in range(len(c_grid["ptbins"]) - 1):
                for j in range(len(c_grid["etabins"]) - 1):
                    ...
                    for sample, hist in h_mc.items():
                        legend_entries = [
                            (hist, "nominal (%s)" % plotter.legend_names[sample], "l")
                        ]
                        for flavour in c_grid["pfflavours"]:
                            h = variation_bands[sample][flavour]

                            legend_entries.append(
                                (h, "#pm 1%% variation %s" % flavour, "f")
                            )
...
if __name__ == "__main__":
    ...
    for selection in ["top", "W"]:
        for c_name, c_grid in category_grids.items():
            ...
            plot_template_variations(
                c_name, c_grid, file_name, selection, output_dir, variation=variation
            )

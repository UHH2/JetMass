...
def draw_lumi(plotpad,
    ...
    if (do_extra_text):
        ...
        extra_x_pos = left_margin*(1+text_padding) if private_work else left_margin + \
            extra_text_rel_X*(1-left_margin-right_margin) if out_of_frame else left_margin*(1+text_padding)
        counter = 1
        if (not out_of_frame):
...
def setup_pads(c, logY: bool = False):
    ...
    if (additional_pad):
        ...
        yratio = 0.25
        yadd = 0.15
        addpad = ROOT.TPad("addpad", "AddPlot", xmin, ymax-yplot-yadd, xmax, ymax-yplot)
    else:
        yplot = 0.7
        yratio = 0.3
        yadd = 0.0
    ...
    if (ratio_plot):
        ...
        ratiopad = ROOT.TPad("ratiopad", "Ratio", xmin, ymax-yplot-yratio-yadd, xmax, ymax-yplot-yadd)
    else:

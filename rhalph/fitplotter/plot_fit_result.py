            ...
            for suffix in ['prefit','postfit']:
                ...
                if(stacked_plot):
                    ...
                    for isignal in range(len(signals)):
                        ...
                        legend.AddEntry(shapes_signal[isignal],signals[isignal]+('*%.1f'%signal_scale if signal_scale>0 else ''),'f')                    
                    stack.Draw('HISTSAME')
                ...
                if(normal_ratio):
                    ...
                    ratioYMax = 1.5*max(abs(ratio_hist_truncated.GetMinimum()),abs(ratio_hist_truncated.GetMaximum()))

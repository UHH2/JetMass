...
def update_nuisance(name, fitargs, nuisances):
    ...
    nuisances[name]["down"] = (
        1
        + (nuisance.getVal() - (nuisance.getAsymErrorLo() if nuisance.getAsymErrorLo() != 0 else nuisance.getErrorLo()))
        * effect
    )
    nuisances[name]["up"] = (
        1
        + (nuisance.getVal() + (nuisance.getAsymErrorHi() if nuisance.getAsymErrorHi() != 0 else nuisance.getErrorHi()))
        * effect
    )
...
def plot_mass_scale_nuisances(config):
    ...
    variation = 0.01
    central = 1.0 + ms_par[:, 1] * variation

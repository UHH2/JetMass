            ...
            for QCD_rho in QCDOrders[1]:
                for DATA_pt in DATAOrders[0]:
                    for DATA_rho in DATAOrders[1]:
                        FTest = isinstance(combine_method, CombineWorkflows) and (
                            combine_method.method == "FTest" or combine_method.method == "FTestBatch"
                        )
                        # print(QCD_pt,QCD_rho,DATA_pt,DATA_rho)
                        QCDOrders_str = "MCTFPt%iRho%i"
                        DataOrders_str = "DataResTFPt%iRho%i"
                        Orders_str = (
                            (QCDOrders_str % (QCD_pt, QCD_rho) + DataOrders_str % (DATA_pt, DATA_rho))
                            if initial_qcd_fit
                            else "TFPt%iRho%i" % (DATA_pt, DATA_rho)
                        )
                        ...
                        if FTest:
                            ...
                            incrPt_config["ModelName"] += (
                                (
                                    QCDOrders_str % (QCD_pt + (1 if do_qcd_scan else 0), QCD_rho)
                                    + DataOrders_str % (DATA_pt + (1 if do_data_scan else 0), DATA_rho)
                                )
                                if initial_qcd_fit
                                else "TFPt%iRho%i" % (DATA_pt + 1, DATA_rho)
                            )
                            ...
                            json.dump(
                                incrPt_config, open(job_workdir + "/" + incrPt_config["ModelName"] + ".json", "w")
                            )

                            incrRho_config = copy.deepcopy(self.config)
                            incrRho_config["ModelName"] += (
                                (
                                    QCDOrders_str % (QCD_pt, QCD_rho + (1 if do_qcd_scan else 0))
                                    + DataOrders_str % (DATA_pt, DATA_rho + (1 if do_data_scan else 0))
                                )
                                if initial_qcd_fit
                                else "TFPt%iRho%i" % (DATA_pt, DATA_rho + 1)
                            )
                            ...
                            json.dump(
                                incrRho_config, open(job_workdir + "/" + incrRho_config["ModelName"] + ".json", "w")
                            )
                        ...
                        if combine_method is not None:
                            if isinstance(combine_method, str):
                                ...
                                cw.workspace = (
                                    job_workdir + "/" + this_config["ModelName"] + "/model_combined.root"
                                )
                                ...
                                if self.fit_qcd_model:
                                    combine_method.workspace = (
                                        job_workdir + "/" + this_config["ModelName"] + "/qcdmodel/model_combined.root"
                                    )
                                else:
                                    combine_method.workspace = (
                                        job_workdir + "/" + this_config["ModelName"] + "/model_combined.root"
                                ...
                                raise (
                                    TypeError(
                                        "combine_method should be either str or CombineWorkflow instance. You provided "
                                        + type(combine_method)
                                    )
                                )
            ...
            for value in values:
                ...
                if combine_method is not None:
                    if isinstance(combine_method, str):
                        ...
                        if self.fit_qcd_model:
                            combine_method.workspace = (
                                self._workdir + "/" + this_config["ModelName"] + "/qcdmodel/qcdmodel_combined.root"
                            )
                        else:
                            combine_method.workspace = (
                                self._workdir
